...
// Параметр раскомментируем только для отладки
//#define TEMP_CODE


#define EXPORT_TIME_LAYERS	// Экспорт временных слоев в TecPlot
//#define EXPORT_ANIMATION	// Экспорт анимации, если есть
//#define EXPORT_DATA_FILE	// Экспорт данных в бинарный файл
...
{ 
  ...
  dT =	beta*dx/cmax				hmax>=epsilon
	...
	*/
	...
	double step_export_graphic = 5; // Шаг по времени экспорта в TecPlot	
	double T_export_graphic_start = 30;	// Время начала экспорта в TecPlot
	double T_export_graphic_end = 55;	// Время окончания экспорта в Tecplot
	...
	#ifdef TEMP_CODE
	//TEMP
	//немного изменем начальные условия в береговых точках
	for (i=0; i<Nx; i++)
	{
		if (H[i] < epsilon && H[i+1] > epsilon)
		{
			H[i]= H[i+1] + B[i+1] - B[i];
		}
		if (H[i] < epsilon && H[i-1] > epsilon)
	...
	while (T_circle < T_add)
	{
		...
		{       
			...
			// Вывод процентного соотношения выволненнных расчетов 
			// в консольную строку			
			...
			//if (number_circle==1)
			{
				...
				// Адрес папки Hill_V20/
				// T0
				Buffer_Name_Tecplot += "Hill_V20/Tecplot T0=";
				...
				*/
				...
				//TEMP
				// Delta = H(t) - H(0)
				N_smb = sprintf( buffer_name,"VARIABLES = X,Ksi,B,H,U,tau,Delta \n");
