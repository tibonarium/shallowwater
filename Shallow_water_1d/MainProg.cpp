...
{ 
  ...
  dT =	beta*dx/cmax				hmax>=epsilon
	...
	*/
	...
	double T_export_graphic = step_export_graphic;
	...
	while (T_circle < T_add)
	{
		...
		{       
			for (i=1; i<(Nx-1); i++)
			{
				...
				// С добавками пропорциональными производной от скорости		
				
				//PT2x2
				//PT2x2 = tau2x2*U2x2*( H2x2*U2x2*( U[i+1] - U[i] )/hx + g*( 0.5*H[i+1]*H[i+1] - 0.5*H[i]*H[i] )/hx + g*H2x2*( B[i+1]-B[i] )/hx - H2x2*Force2x2 ) + g*tau2x2*( U2x2*( 0.5*H[i+1]*H[i+1]-0.5*H[i]*H[i] )/hx + H2x2*H2x2*( U[i+1]-U[i] )/hx ) + (2*g/3)*tau2x2*H2x2*H2x2*( U[i+1]-U[i] )/hx;
				
				//PT2x1
				//PT2x1 = tau2x1*U2x1*( H2x1*U2x1*( U[i] - U[i-1] )/hx + g*( 0.5*H[i]*H[i] - 0.5*H[i-1]*H[i-1] )/hx + g*H2x1*( B[i]-B[i-1] )/hx - H2x1*Force2x1 ) + g*tau2x1*( U2x1*( 0.5*H[i]*H[i]-0.5*H[i-1]*H[i-1] )/hx + H2x1*H2x1*( U[i]-U[i-1] )/hx ) + (2*g/3)*tau2x2*H2x2*H2x2*( U[i]-U[i-1] )/hx;
			...
			//if (number_circle==1)
			{
				...
				Buffer_Name_Tecplot += "Tecplot T0=";
					p_number = gcvt((T_export_graphic+T_current)/60,NDEC,buffer_name);
				...
				*/
				...
				// 
				T_export_graphic += step_export_graphic;
