...
{ 
  ...
  dT =	beta*dx/cmax				hmax>=epsilon
	...
	*/
	...
	double T0 = 120; // 3300 s
	double alpha = 0.8; // 0.1 
	double beta = 0.1; // 0.2
	...
	double step_export_graphic = 10; //2*60 // 4s 35 sec*24 = 840 cards -> *4 = 3360 sec 
	
	double T_export_graphic_start = 10;
	double T_export_graphic_end = 120;
		...
		{       
			for (i=1; i<(Nx-1); i++)
			{
				...
				//if (fabs(Ut[i]) > 50)
				{
					...
					Temp_variable = ( H[i+1]*U[i+1]*U[i+1] - H[i]*U[i]*U[i] )/hx;	
					cout << Temp_variable << endl;

					cout << H[i+1]*U[i+1]*U[i+1] << "\n" << H[i]*U[i]*U[i] << endl;

					cout << "\nH\n" << H[i+1] << "\n" << H[i] << endl;

					cout << "\nU\n" << U[i+1] << "\n" << U[i] << endl;

					Temp_variable = g*( 0.5*H[i+1]*H[i+1] - 0.5*H[i]*H[i] )/hx;	
					cout << "\n" << Temp_variable << endl;

					Temp_variable = g*H2x2*( B[i+1] - B[i] )/hx;	
					cout << Temp_variable << endl;

					Temp_variable = H2x2*Force2x2;	
					cout << Temp_variable << endl;
				...
				if (Ht[i] > epsilon)
				{
					...
					*/
					...
					if (Ut[i] < -50)
					{
						...
						cout << ( PT2x2-PT2x1 ) << endl;
						cout << (U2x2*J2x2-U2x1*J2x1) << endl;
						cout << ( 0.5*H2x2*H2x2-0.5*H2x1*H2x1 ) << endl;
						cout << ( 0.5*(H2x2+H2x1) - tau[i]*( H2x2*U2x2-H2x1*U2x1 )/hx ) << endl;
			...
			if (T_circle <= 150)
			{
				...
				else
				{
					...
					{
						H_r = 50*sin(pi*(T_circle+30-120)/60) + 25;
					}
				}

				Ht[0] = Ht[1];      Ht[Nx-1] = 2*H_r - Ht[Nx-2];
			...
			if (number_circle==1 || number_circle==2)
			{
